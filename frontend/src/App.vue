...
const productForm = reactive({ product_key: '', name: '', device_type: 'sensor', description: '' })
...
async function loadData({ silent = false } = {}) {
  ...
  errorMessage.value = ''
  try {
    const [productPayload, devicePayload, alarmPayload] = await Promise.all([
    ...
    if (selectedDeviceId.value) await loadTelemetry(selectedDeviceId.value)
...
async function sendCommand() {
  ...
  try {
    await api.post(`/api/devices/${encodeURIComponent(commandDevice.value.device_id)}/commands`, { method: commandForm.method, params })
  ...
  <div class="app-shell">
    ...
    <main class="workspace">
      ...
      <section v-else-if="activeView === 'devices'" class="content-area">
        <div class="page-intro"><div><span class="eyebrow">ASSET / DEVICES</span><h1>设备管理</h1><p>管理设备身份、连接状态和远程操作。</p></div><button class="primary-button" @click="openDeviceModal"><Plus :size="17" />注册设备</button></div>
    ...
    <div v-if="showProductModal" class="modal-backdrop" @click.self="showProductModal = false"><section class="modal"><div class="modal-heading"><div><span class="panel-kicker">NEW PRODUCT</span><h2>新建产品</h2></div><button class="icon-button" title="关闭" @click="showProductModal = false"><X :size="18" /></button></div><div class="form-grid"><label>产品名称<input v-model="productForm.name" placeholder="例如：温湿度传感器" /></label><label>设备类型<select v-model="productForm.device_type"><option value="sensor">传感器</option><option value="actuator">执行器</option><option value="composite">复合设备</option></select></label><label>产品 Key（可选）<input v-model="productForm.product_key" placeholder="系统自动生成" /></label><label>描述（可选）<input v-model="productForm.description" placeholder="产品用途" /></label></div><div class="modal-actions"><button class="ghost-button" @click="showProductModal = false">取消</button><button class="primary-button" @click="createProduct"><Plus :size="16" />创建产品</button></div></section></div>
